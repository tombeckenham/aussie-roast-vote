...
async function importAECDivisions() {
  try {
    console.log('🔄 Starting import of AEC divisions into electoral_seats table...');
    ...
    const existingSeats = await db.select({ name: electoralSeats.name }).from(electoralSeats);
    const existingSeatNames = new Set(existingSeats.map(seat => seat.name.toUpperCase()));
    
    console.log(`ℹ️ Found ${existingSeatNames.size} existing electoral seats in database`);
    ...
    for (const division of divisions) {
      try {
        ...
        if (locationDesc.includes('urban') || locationDesc.includes('metropolitan')) {
        ...
        console.error(`❌ Error importing division ${division.Name}:`, error.message);
